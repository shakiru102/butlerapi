...
import { validateSignupAdminSchema, validateSubcriptionUpdate, validateUpdateStatus } from "../utils/validate"
...
export const adminUpdateSubscriptionValidation = async ( req: Request, res: Response, next: NextFunction) => {
    try {
        const order = await Order.findById({ _id: req.params.orderID })
    if(!order) throw new Error('Invalid order id parameter')
    const { error } = validateSubcriptionUpdate(req.body)
    if(error) return res.status(400).send(error.details[0].message)
    next()
    } catch (error: any) {
        return res.status(400).send({ status: 'Failed', error: 'Invalid order id parameter' })
